...
- **内置预设**（一键套用）：
  ...
  - 📘 党政机关公文（GB/T 9704-2012）—— A4 版心 156×225 mm，每页 22 行 × 每行 28 字（文档网格），仿宋_GB2312 三号，2 号小标宋标题，黑体 / 楷体 / 仿宋三层标题，4 号宋体页码单页居右、双页居左
  - 📘 毕业论文（通用）—— 宋体小四 + Times New Roman、黑体分级标题
  - 📘 简洁现代报告 —— 微软雅黑，适合内部汇报材料
- **可配置项**：
  - 页面：纸张（A4/B5/Letter/16K）、方向、四边页边距、页眉页脚距边界、文档网格（无 / 指定行和字符网格 / 保持原文档）
...
- 排版引擎直接操作 `.docx` 的 OOXML（`app/engine.py`）：
  ...
  - 无网格模式：固定行距同时关闭段落"对齐到文档网格"（`w:snapToGrid`）与文档网格（`w:docGrid`），否则 WPS 中行距会失真
  - 网格模式：`w:docGrid type="linesAndChars"`，`linePitch` = 版心高度 ÷ 行数（缇），`charSpace` = (字符间距 − 正文字号) × 4096（向下取整，否则 Word 每行少 1 字）；段落对齐网格、单倍行距；比正文大的标题不对齐字符网格
...
| 5.3 | 一般用 3 号仿宋体，每面 22 行、每行 28 字，撑满版心 | 文档网格"指定行和字符网格"22 行 × 28 字（与 Word 页面设置写出的 XML 一致），正文仿宋_GB2312 三号 |
...
| 7.3.3 | 正文每个自然段左空二字；层次序数第一层黑体、第二层楷体、第三 / 四层仿宋 | 首行缩进 2 字（网格模式下按网格间距精确换算，避免 Word 把首字挤到第 3 格）；2 级黑体、3 级楷体_GB2312、4 级仿宋_GB2312，均三号不加粗、缩进 2 字 |
...
用本机 Word / WPS 实际排版并核对上述指标（每页行数、每行字数、标题字体、页码位置与奇偶对齐）：
...
- 文档网格模式下行距由网格决定，"正文"页里的行距值仅在关闭网格时使用
